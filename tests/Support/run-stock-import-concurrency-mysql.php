...
if (! in_array($host, ['127.0.0.1', 'localhost', '::1'], true) || app()->environment('production')) {
    fwrite(STDERR, "Refused: this regression may only use a local, non-production MySQL/MariaDB server.\n");
    exit(2);
}
...
$dsn = sprintf('mysql:host=%s;port=%d;dbname=%s;charset=%s', $host, (int) ($config['port'] ?? 3306), $database, (string) ($config['charset'] ?? 'utf8mb4'));
$pdo = new PDO($dsn, (string) ($config['username'] ?? ''), (string) ($config['password'] ?? ''), [PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION]);
$prefix = '';
$prefixQuery = $pdo->prepare('SELECT COUNT(*) FROM information_schema.TABLES WHERE TABLE_SCHEMA = ? AND LEFT(TABLE_NAME, 2) = ?');
$start = random_int(0, 255);

for ($offset = 0; $offset < 256; $offset++) {
    $candidate = sprintf('%02x', ($start + $offset) % 256);
    $prefixQuery->execute([$database, $candidate]);

    if ((int) $prefixQuery->fetchColumn() === 0) {
        $prefix = $candidate;
        break;
    }
}
...
try {
    ...
    foreach ($migrations as $migration) {
        $process = new Process([PHP_BINARY, 'artisan', 'migrate', '--force', '--no-interaction', '--path='.$migration], $root, $environment);
    ...
    $test = new Process([PHP_BINARY, 'artisan', 'test', 'tests/Integration/StockImportSnapshotConcurrencyTest.php'], $root, $environment);
    $test->setTimeout(180);
